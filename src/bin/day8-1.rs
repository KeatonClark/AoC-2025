...
fn calculate(
	count: usize,
	mut edges: BinaryHeap<Reverse<Edge>>,
	points: Vec<Point>,
) -> Vec<HashSet<Point>> {
...
fn main() {
	...
	let mut groups = calculate(1000, edges, points);
	groups.sort_by(|groupa, groupb| groupa.len().cmp(&groupb.len()));
	println!(
		"{}",
		groups[groups.len() - 3..]
			.iter()
			.fold(1, |acc, group| acc * group.len())
	);
}
...
425,690,689"
		...
		let mut groups = calculate(10, edges, points);
